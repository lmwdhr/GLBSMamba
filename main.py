...
from model.GLT_Net.utils import train_patch
from utils import (
    metrics,
    convert_to_color_,
    convert_from_color_,
...
# 将print输出重定向到文件
sys.stdout = open('./results/Houston2013_Multimodality_Mamba111_59.txt', 'w')##Ablation2_3direction
...
parser.add_argument(
    "--dataset", type=str, default='Houston2013', choices=dataset_names,
    help="Dataset to use."
          "Houston2013, Trento, MUUFL, Augsburg"
)
parser.add_argument(
    "--applyPCA", type=bool,
...
for run in range(N_RUNS):
    seed_torch(seed=seed[run])
    if TRAIN_GT is not None and TEST_GT is not None:
        train_gt = open_file(TRAIN_GT)['train_data']#['TRLabel']for Houston2013
        test_gt = open_file(TEST_GT)['test_data']#['TRLabel']for Houston2013
