...
class hsiMamba(BaseBackbone):
    ...
    def __init__(self,
        ...
        self.weights = nn.Parameter(torch.zeros(1, 8, 1))
    ...
    def _format_output(self, x, hw):
        ...
        if self.out_type == 'featmap':
            if 'feature' in self.arch:
                B = x.size(0)
                x = patch_token.reshape(B, *hw, -1).permute(0, 3, 1, 2)
                x = self.tokenlearner(x)
                x = self.ln3(x)  ######层归一化
                H = self.patch_resolution[0] - 2
                return x.reshape(B, H, H, -1).permute(0, 3, 1, 2)
            elif 'view' in self.arch:
                B = x.size(0)
...
class GlobalLocalBlock(nn.Module):
    def __init__(self, img_size, in_channels, out_channels):
        ...
        if img_size == 9:
            ...
            self.global_feature = hsiMamba(arch='globalfeature1', img_size=img_size,
                                           patch_size=1, in_channels=in_channels, out_type='featmap',
                                           path_type=path_type, patch_cfg=dict(stride=1))
        if img_size == 7:
            path_type = path[1]
            self.global_view = hsiMamba(arch='globalview2', img_size=img_size,
                                        patch_size=1, in_channels=in_channels, out_type='featmap',
                                        path_type=path_type, patch_cfg=dict(stride=1))
            self.global_feature = hsiMamba(arch='globalfeature2', img_size=img_size,
                                           patch_size=1, in_channels=in_channels, out_type='featmap',
                                           path_type=path_type, patch_cfg=dict(stride=1))
    ...
    def forward(self, hsi):
        ...
        global_feature = self.global_feature(global_view)[0]
        local_feature = self.local_feature(global_view)

        feature_map = self.FusionLayer(global_feature, local_feature)
...
class GLfusionBlock(nn.Module):
    ...
    def forward(self, x1, x2):#x1:global x2:local
        localf = x1 + x2
        globalf_enhancement = self.cross_attention(x1, x2, x2)
        globalf = globalf_enhancement + x1
